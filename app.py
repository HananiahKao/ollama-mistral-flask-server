...
def run_generation_task(task_id, context):
    try:
        ...
        # If Ollama is not available, use fallback template
        if err or not html:
            if "Ollama service not available" in str(err) or "404" in str(err):
                print(f"[INFO] Task {task_id}: Using fallback template due to Ollama unavailability")
                html = render_template('fallback.html', context=context)
            else:
